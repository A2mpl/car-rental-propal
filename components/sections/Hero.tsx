'use client';
import { AnimatePresence, type BezierDefinition, motion, type Transition, type Variants } from 'framer-motion';
...
export default function Hero() {
  ...
  const next = (): void => setCurrent(([i]: [number, number]): [number, number] => [(i + 1) % models.length, 1]);
  const prev = (): void =>
        ...
        <div className={styles.carContent}>
          <AnimatePresence mode="wait" custom={dir}>
            ...
            >
              ...
              <Image src={model.hero} alt={model.name} fill className={styles.carImg} priority unoptimized />
            </motion.div>
          </AnimatePresence>
        </div>
      </div>
        ...
        <div className={styles.bottomBarInner}>
          <div className={styles.thumbBox}>
            <AnimatePresence mode="wait">
              ...
              >
                <Image src={model.thumb} alt={model.name} fill className={styles.carImg} unoptimized />
              </motion.div>
            </AnimatePresence>
          </div>
          ...
          <div className={styles.metaData}>
            <div className={styles.titleRow}>
              ...
              <div className={styles.arrows}>
                <button className={styles.arrowBtn} onClick={prev} aria-label="Previous model" type="button">
                  ←
                </button>
                <button className={styles.arrowBtn} onClick={next} aria-label="Next model" type="button">
                  →
                </button>
              </div>
            </div>
